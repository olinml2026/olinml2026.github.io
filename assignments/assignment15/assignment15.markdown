...
<div>
$$
\text{cosine\_similarity}(x_q, x_i) = \frac{\bm{\phi}(x_q)^\top \bm{\phi}(x_i)}{\|\bm{\phi}(x_q)\| \|\bm{\phi}(x_i)\|}
$$
</div>
...
recall 
some [facts about the dot product](https://en.wikipedia.org/wiki/Dot_product#Geometric_definition)).
